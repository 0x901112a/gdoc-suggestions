...
export async function login(userDataDir = DEFAULT_USER_DATA_DIR) {
  ...
  // Verify login actually succeeded by navigating to Google Docs
  ...
  if (finalUrl.includes('accounts.google.com') || finalTitle.includes('Sign-in') || finalTitle.includes('Sign in')) {
    await context.close();
    console.error('Login failed — Google sign-in was not completed within the time limit.');
    console.error('Run `gdoc-suggest login` again and complete the sign-in flow in the browser.');
    process.exit(1);
  }
...
export async function launchHeadless(userDataDir = DEFAULT_USER_DATA_DIR) {
  ...
  // Check if the session landed on a Google sign-in page (auth expired)
  const page = context.pages()[0] || await context.newPage();
