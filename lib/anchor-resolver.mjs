...
/**
 * Find which occurrence number (1-based) a substring at a given position is.
 */
...
/**
 * Find the shortest unique substring of `text` that contains the character at `centerIndex`.
 * Expands outward from centerIndex, preferring to include surrounding context.
 * Returns { query, startPos } or null if no unique substring fits within MAX_FIND_LENGTH.
 */
...
export function resolveInsert(docText, index) {
  ...
  // Fallback: if no unique substring found, use the text at the index
  // with match numbering
...
export { countOccurrences, getMatchNumber, findShortestUniqueAround, MAX_FIND_LENGTH };
