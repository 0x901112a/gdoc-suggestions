...
export async function navigateToDoc(page, docUrl) {
  ...
  // Wait for the mode switcher button — indicates doc UI is fully loaded
...
export async function switchToSuggestingMode(page) {
  ...
  // Wait for the Suggesting option to appear
  ...
  // Wait for the button text to confirm mode changed
...
export async function findAndSelect(page, query, matchNum = 1) {
  ...
  // Wait for Find input to appear (it's an input inside a toolbar)
  await page.locator('[aria-label="Find in document…"], [aria-label="Find in document"], input[name="Find"]').first()
    ...
    .catch(() => page.waitForTimeout(500)); // fallback if selector doesn't match
...
export async function findAndReplace(page, findText, replaceText, matchNum = 1) {
  ...
  // Wait for the Replace button to become enabled (disabled until a match is found)
  ...
  // Poll until the button is enabled (not disabled)
  await page.waitForFunction(
    (btnName) => {
    ...
    'Replace',
    { timeout: WAIT_TIMEOUT }
  ).catch(() => page.waitForTimeout(500)); // fallback
...
export async function setTextColor(page, colorName) {
  ...
  // Wait for the color picker grid to appear
...
export async function replaceSelectedSuggestion(page, text, style) {
  ...
  // Type the first character — replaces the selection and creates the suggestion
  await page.keyboard.type(text[0]);
  await page.waitForTimeout(DELAY.short);

  // Insert the rest instantly
  if (text.length > 1) {
...
export async function rejectAllSuggestions(page) {
  ...
  while (true) {
    ...
    // Wait for the suggestion card to disappear
